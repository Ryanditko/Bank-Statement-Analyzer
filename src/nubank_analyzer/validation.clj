(ns nubank-analyzer.validation
  "Módulo de validação com Clojure Spec
   Valida transações, datas, valores e estruturas de dados"
...
(defn valid-date-format?
  "Verifica se string é uma data válida em formato conhecido"
  [date-str formats]
  (boolean
    (some (fn [fmt]
            (try
...
(defn validate-amount
  ...
  (cond
    (nil? amount)
    {:valid? false :error "Valor ausente"}
    
    (not (number? amount))
    {:valid? false :error "Valor não é um número"}
    
    (Double/isNaN amount)
    {:valid? false :error "Valor inválido (NaN)"}
    
    (Double/isInfinite amount)
    {:valid? false :error "Valor infinito"}
...
(defn validate-transactions
  "Valida lista de transações e retorna relatório"
  [transactions date-formats]
  (log/info "Validando %d transações..." (count transactions))
  ...
  (let [results (map-indexed 
    ...
    (log/info "Validação concluída: %d válidas, %d inválidas" 
...
(defn validate-csv-headers
  "Valida se CSV tem os headers necessários"
  [headers]
...
(defn detect-outliers
  "Detecta valores outliers usando método IQR"
  [amounts threshold]
...
(defn validate-amounts-distribution
  "Valida distribuição de valores procurando anomalias"
  [transactions]
  (let [amounts (map :amount transactions)
    ...
    (when (> outlier-count 0)
      (log/warn "Detectados %d valores outliers" outlier-count))
...
(defn sanitize-description
  "Remove caracteres especiais e normaliza descrição"
  [desc]
  (-> desc
      str/trim
      (str/replace #"\s+" " ")
...
(defn sanitize-transaction
  "Sanitiza campos de uma transação"
  [transaction]
  (-> transaction
